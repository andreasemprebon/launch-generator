...
tree = etree.parse('../ocarina-ros/ever_xml.xml')
...
for process in processes:
    ...
    identifier  = process.find('identifier').text
    category    = process.find('category').text
    namespace   = process.find('namespace').text

    launch_node_properties = {  'name'  : identifier,
                                'pkg'   : namespace,
                                'type'  : name  }
    new_launch_node = etree.SubElement(launch_file_root, 'node', launch_node_properties)


    print(name, identifier, category, namespace)
